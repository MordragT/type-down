...
pub trait Visitor: Debug {
    ...
    fn visit_doc(&self, state: &mut Self::State, doc: Document) {
        walk_doc(self, state, doc)
    }
    ...
    fn visit_content(&self, state: &mut Self::State, content: Content) {
        walk_content(self, state, content)
    }
    ...
    fn visit_ref(&self, _state: &mut Self::State, _ref: Ref) {}
    ...
    fn visit_block_expr(&self, state: &mut Self::State, block: ExprBlock) {
        walk_block_expr(self, state, block)
    }

    fn visit_call_expr(&self, _state: &mut Self::State, _call: Call) {}

    fn visit_text(&self, _state: &mut Self::State, _text: Text) {}

    // fn visit_content_expr(&self, _state: &mut Self::State, _content: Content) {}

    // fn visit_error(&self, _state: &mut Self::State, _span: Error) {}
}

pub fn walk_doc<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, doc: Document) {
    for block in doc {
...
pub fn walk_heading<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, heading: Heading) {
    visitor.visit_content(state, heading.content())
}
...
pub fn walk_table_row<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, table_row: TableRow) {
    for cell in table_row {
        visitor.visit_block(state, cell);
    }
}

pub fn walk_term<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, terms: Terms) {
    for item in terms {
...
pub fn walk_term_item<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, item: TermItem) {
    visitor.visit_content(state, item.term());
    visitor.visit_content(state, item.desc())
}
...
pub fn walk_content<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, content: Content) {
...
pub fn walk_inline<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, inline: Inline) {
    match inline {
        ...
        Inline::Ref(r) => visitor.visit_ref(state, r),
...
pub fn walk_code<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, code: Code) {
    visitor.visit_expr(state, code.get())
}
...
pub fn walk_expr<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, expr: Expr) {
    match expr {
        ...
        Expr::Content(content) => visitor.visit_content(state, content),
    }
}

pub fn walk_block_expr<V: Visitor + ?Sized>(visitor: &V, state: &mut V::State, block: ExprBlock) {
    for expr in block {
